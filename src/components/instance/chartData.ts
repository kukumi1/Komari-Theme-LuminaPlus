...
interface AlignedBucketReducer {
  ...
  /**
   * 可选：桶内原始采样的统计，供 tooltip 说明这一格背后压了多少次探测。
   * 不实现就不产出 buckets——加权路径(丢包率)没有"这一格最快/最慢多少毫秒"的语义。
   */
  getStat?: (seriesIndex: number, bucket: number) => MetricBucketStat | null;
}
...
function downsampleAlignedWith(
  ...
  const outBuckets = reducer.getStat
    ? perTask.map(() => [] as Array<MetricBucketStat | null>)
    : null;
  for (let bucket = 0; bucket < maxPoints; bucket += 1) {
    if (timeCount[bucket] === 0) continue;
    outTimes.push(timeSum[bucket] / timeCount[bucket]);
    for (let seriesIndex = 0; seriesIndex < seriesCount; seriesIndex += 1) {
      // 统计与显示值分开取：桶里既有丢包又有正常值时线要断开(null)，
      // 但"这一格最快/最慢多少"仍然是有意义的信息。
      outBuckets?.[seriesIndex].push(reducer.getStat?.(seriesIndex, bucket) ?? null);
...
export function downsampleAligned(
  ...
  return downsampleAlignedWith(times, perTask, maxPoints, (seriesCount, bucketCount) => {
    ...
    // 极值一律记录：保峰模式用它判断并输出尖峰，其余模式用它产出 tooltip 的桶内统计。
    const valueMax = Array.from({ length: seriesCount }, () =>
      new Array<number>(bucketCount).fill(Number.NEGATIVE_INFINITY),
    );
    const valueMin = Array.from({ length: seriesCount }, () =>
    ...
    return {
      addValue(seriesIndex, bucket, _pointIndex, value) {
        ...
        if (value > valueMax[seriesIndex][bucket]) {
          valueMax[seriesIndex][bucket] = value;
        }
        if (value < valueMin[seriesIndex][bucket]) {
      ...
      getStat(seriesIndex, bucket) {
        const count = valueCount[seriesIndex][bucket];
        if (count === 0) return null;
        return {
          min: valueMin[seriesIndex][bucket],
          max: valueMax[seriesIndex][bucket],
          avg: valueSum[seriesIndex][bucket] / count,
          count,
        };
      },
    };
